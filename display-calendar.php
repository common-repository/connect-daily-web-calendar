...
class CDCalendarWriter extends CDailyBaseRenderer {
     ...
     * @param $item 
     */
    private function render_item($item,$timed) {
        $result = "<span class=\"CDEventTitle".($timed ? '' : ' CDUntimed')."\" ";
        if (property_exists($item, 'cal_item_id')) {
        ...
        $result .= (empty($item->location_id) ? '' : '" data-location_id="'.$item->location_id)
    ...
    private function render_single_day($date, $items) {
        ...
        foreach ($items as $item) {
            if ($this->doesRangeOverlap($aRange, array($item->occurrenceStart - $tzOffset * Time_Data::MILLIS_SECOND, $item->occurrenceEnd - $tzOffset * Time_Data::MILLIS_SECOND))) {
                ...
                $temp .= $this->render_item($item,$bTimed);
    ...
    function iFrameTag($atts, $content = null) {
        ...
        if (empty($cdaily_url)) {
            return '<h1>Notice</h1>You must go to the settings tab for connectDaily and enter the URL to the calendar.';
        }
        $cdaily_url .= $view;
        if ($options != '') {
            $cdaily_url .= '?' . $options;
    ...
    public function renderSpecificDay($fields) {
        foreach (array(
            ...
            'show_resources' => '0',
            'dayspan' => '0',
            'start' => CDailyPlugin::digitsOnly($fields['date'])
            ) as $key => $default) {
        ...
        $json_url = 'jsonp/' . CDailyPlugin::filterByMethod($fields['by_method']) . '/' . intval($fields['by_id']) . '.js';
    ...
    public function renderiCal($atts, $content = null) {
        ...
        if ($content != null) {
            if ($content=='AUTO') {
                $content="<img class=\"CDNavIcon\" src=\"".$this->plugin->getIconURL('COM_CONNECTDAILY_CALENDARICON')."\">";
            }
            $result .= $content;
        }
        $result .= '</a>';
    ...
    public function renderMonth($args, $bare_calendar = false) {
        if (!isset($args['by_id'])) {
            // This can happen if gutenberg get's a little confused.
            $this->plugin->debugOut('renderMonth() args do not contain by_id',$args);
            return;
        }
        foreach (array(
              'allow_duplicates' => '1',
        ...
        $json_url = 'jsonp/' . CDailyPlugin::filterByMethod($args['by_method']) . '/' . intval($args['by_id']) . '.js';
        if ($bare_calendar) {
            
            /*
                connectDaily doesn't actually use this paramter,
        ...
        $addEventLink='';
        if ($this->plugin->isAdmin() && !$bare_calendar && $args['show_add_button']) {
            $addEventLink = '<a id="CDInternalAddEventLink" href="javascript:CDaily.doSingleSignon();"><button>' . $this->plugin->translate('COM_CONNECTDAILY_AddEvents') . '</button></a>';
        }
        if (!$reqData->wasSuccess()) {
            return $reqData->getErrorText();
        } else if ($reqData->from_cache) {
            return $addEventLink.$reqData->content;
        }
        extract($args);
        $result = "";
...
jQuery(document).ready(function(){
    ...
    */
    ...
    if (!CDaily.hasOwnProperty('navIconHandler')){
        CDaily.navIconHandler=[];
        CDaily.navIconHandlerTarget=[];
    }

    CDaily.navIconHandler.push(function(){
    ...
    CDaily.navIconHandlerTarget.push(container_id);
    jQuery(container_id+" .CDNavIcon").click(CDaily.navIconHandler[CDaily.navIconHandler.length-1]); // This binds the on-clicks for the arrows.     
...
EOF;
    ...
    public function renderMiniCalendar($args,$id='cdaily-minicalendar'){
        ...
        */
        ...
        $json_url='jsonp/'.CDailyPlugin::filterByMethod($args['by_method']).'/daymap/'.intval($args['by_id']).'.js';
